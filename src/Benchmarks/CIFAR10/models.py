...
def hpS(with_reg):	
	...
	for r in range(len(regularization_amount)):
		for l in range(len(learning_rate)):
			...
			tf.keras.layers.Conv2D(32,  kernel_size = 3, activation='relu', input_shape = (32, 32, 3), kernel_regularizer=regularization_amount[r]),
...
def PD(with_reg):
	...
	model_num = "1 no_reg CIFAR10"
	model = tf.keras.Sequential([
	tf.keras.layers.Conv2D(32,  kernel_size = 3, activation='relu', input_shape = (32, 32, 3), kernel_regularizer=regularizer),
